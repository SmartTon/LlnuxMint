...
public class WebAura extends Module {
    ...
    public WebAura() {
        super(Categories.ExtraLint, "web-aura", "Puts webs instead of surround.");
    }
    ...
    private void onTick(TickEvent.Pre event) {
        if (selfToggle.get() && placed && placePositions.isEmpty()) {
         ...
         if (InvUtils.findItemInHotbar(Items.COBWEB) == -1) {
            placePositions.clear();
            placed = false;
            return;
        }
        ...
        if (timer >= delay.get() && placePositions.size() > 0) {
            ...
            if (BlockUtils.place(blockPos, Hand.MAIN_HAND, InvUtils.findItemInHotbar(Items.COBWEB), rotate.get(), 50, true)) {
