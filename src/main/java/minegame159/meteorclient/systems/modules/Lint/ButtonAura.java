...
public class ButtonAura extends Module {
    ...
    private final Setting<BottomMode> bottomPlacement = sgGeneral.add(new EnumSetting.Builder<BottomMode>()
            ...
            .defaultValue(BottomMode.Surround)
            .build()
    );
    ...
    public ButtonAura() {
        super(Categories.ExtraLint, "button-aura", "Puts buttons instead of surround.");
    }
    ...
    private void onTick(TickEvent.Pre event) {
        if (selfToggle.get() && placed && placePositions.isEmpty()) {
         ...
         if (InvUtils.findItemInHotbar(Items.POLISHED_BLACKSTONE_BUTTON) == -1) {
            placePositions.clear();
            placed = false;
            return;
        }
        ...
        if (timer >= delay.get() && placePositions.size() > 0) {
            ...
            if (BlockUtils.place(blockPos, Hand.MAIN_HAND, InvUtils.findItemInHotbar(Items.POLISHED_BLACKSTONE_BUTTON), rotate.get(), 50, true)) {
    ...
    private void fillPlaceArray(PlayerEntity target) {
        ...
        switch (bottomPlacement.get()) {
            ...
            case longSurround:
                ...
                add(targetPos.add(0, 0, 1));
                add(targetPos.add(0, 0, -1));
                add(targetPos.add(2, 0, 0));
                add(targetPos.add(-2, 0, 0));
                add(targetPos.add(0, 0, 2));
                add(targetPos.add(0, 0, -2));
                break;
            case fullSurround:
                add(targetPos.add(1, 0, 0));
