...
public class TNTPlace extends Module {
    ...
    public TNTPlace() {
        super(Categories.Mint, "tnt-place", "place tnt on player top");
    }
    ...
    private void onTick(TickEvent.Pre event) {
        ...
        if (InvUtils.findItemInHotbar(Items.TNT) == -1) {
            placePositions.clear();
            placed = false;
            return;
        }
        ...
        if (timer >= delay.get() && placePositions.size() > 0) {
            ...
            if (BlockUtils.place(blockPos, Hand.MAIN_HAND, InvUtils.findItemInHotbar(Items.TNT), rotate.get(), 50, true)) {
